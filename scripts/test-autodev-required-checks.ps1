...
$repo = 'test/repo'
$cases = @(
  @{ name = '単一チェック'; json = '{"contexts":["check"],"checks":[]}'; expected = 'check' },
  @{ name = '複数チェックと明示指定の結合'; json = '{"contexts":["check","lint"],"checks":[]}'; explicit = @('check','audit'); expected = 'check,lint,audit' },
  ...
  @{ name = '設定なしでも明示指定を検証'; json = '{"contexts":[],"checks":[]}'; explicit = @('check'); expected = 'check' }
)
