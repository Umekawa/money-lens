...
assert.ok(app.includes("const height=Math.abs(value)/max*50"), "棒高は金額に比例させる");
assert.ok(!app.includes("Math.max(4,Math.abs(value)/max*50)"), "非ゼロ棒の最小高さを設けない");
...
assert.ok(app.includes("value<0?'top:50%;bottom:auto;':'top:auto;bottom:50%;'"), "正負の棒をゼロ線の反対側に描く");
