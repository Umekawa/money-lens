...
ヘッダーの空白は無視し、資産の合計列は `合計（円）` のような単位付き表記にも対応します。日付は `YYYY-MM-DD` または `YYYY/M/D`（区切りは混在不可）の実在する日付、金額は符号・3桁区切り・`円`/`¥`/`￥`付きの数値を使用してください。文字コードは UTF-8 または Shift_JIS に対応します。必須列がない行や、日付・金額・資産内訳が不正な行は取り込まず、ファイル名と行番号を画面に表示します。CSV以外のファイルはスキップします。
